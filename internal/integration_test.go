...
func TestIntegration_TaskListingWithFilters(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tasksInterface, ok := response["tasks"].([]interface{})
			if !ok {
				t.Fatalf("Expected tasks to be []interface{}, got %T", response["tasks"])
			}
			...
			if len(tt.expectedStatus) > 0 {
				for _, taskInterface := range tasks {
					taskInterface, ok := taskInterface.(map[string]interface{})
					if !ok {
						t.Fatalf("Expected task to be map[string]interface{}, got %T", taskInterface)
					}
					task := taskInterface

					statusInterface, ok := task["status"].(string)
					if !ok {
						t.Fatalf("Expected status to be string, got %T", task["status"])
					}
					actualStatus := statusInterface
